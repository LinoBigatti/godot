...
String MultiSpinBox::get_default_string() const {
	...
	if (range[0] == range[1]) {
		...
		if (inner_property_name.size() != 0) default_string = vformat("%s.%s", default_string, inner_property_name);
	}
...
void MultiSpinBox::_text_entered(const String &p_string) {
	...
	if (p_string == get_range_string()) return;
	...
	for (int i = 0; i < inputs.size(); i++) {
		...
		if (value.get_type() == Variant::REAL or value.get_type() == Variant::INT) {
			value = CLAMP((double)value, min, max);

			if (inner_property_name.size() != 0) {
				Variant original_property = input.get_named(property_name);
				original_property.set_named(inner_property_name, value);

				inputs_copy[i].set_named(property_name, original_property);
			} else {
				inputs_copy[i].set_named(property_name, value);
			}
...
Ref<Texture> MultiSpinBox::get_error_or_warning_icon() {
	if (error != OK) {
...
void MultiSpinBox::change_values(double by) {
	for(int i = 0; i < inputs.size(); i++) {
		...
		if (inner_property_name.size() != 0) {
			...
			inputs[i].set_named(property_name, original_property);
		} else {
			inputs[i].set_named(property_name, value);
		}
	}
...
void MultiSpinBox::reset_values() {
	...
	for (int i = 0; i < inputs.size(); i++) {
		Variant input = inputs[i];
		Variant property = input.get_named(property_name);
		if (inner_property_name.size() != 0) {
			values[i] = property.get_named(inner_property_name);
		} else {
			values[i] = property;
		}
	}
...
void MultiSpinBox::_line_edit_focus_enter() {
	...
	line_edit->set_cursor_position(default_string.size());
}
...
inline void MultiSpinBox::_adjust_width_for_icon(const Ref<Texture> &updown, const Ref<Texture> &warning_error) {
...
void MultiSpinBox::_notification(int p_what) {
	if (p_what == NOTIFICATION_DRAW) {
		...
		Ref<Texture> error_warning = get_error_or_warning_icon();
		...
		_adjust_width_for_icon(get_icon("updown"), get_error_or_warning_icon());
...
void MultiSpinBox::set_align(LineEdit::Align p_align) {
	line_edit->set_align(p_align);
}

LineEdit::Align MultiSpinBox::get_align() const {
